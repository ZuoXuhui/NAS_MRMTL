...
class SegTaskNet(nn.Module):
    def __init__(self, cfg, norm_layer=nn.BatchNorm2d):
        ...
        self.task = SegTask(cfg.num_classes, cfg.datasets.ignore_index)
...
class NDDRTaskNet(nn.Module):
    def __init__(self, cfg, norm_layer=nn.BatchNorm2d):
        ...
        ## task auxiliary training
        self.task1 = SegTaskNet(cfg, norm_layer=norm_layer)
        self.task2 = FusionTaskNet(cfg, norm_layer=norm_layer)
        ...
        self.in_chans = cfg.in_chans
        # encoder
        embed_dims = cfg.encoder.embed_dim

        self.task1.brige = nn.ModuleList([])
        self.task2.brige = nn.ModuleList([])
        # nddr
        nddrs = []
        for stage_id in range(self.num_stages):
    ...
    def init_weigths(self, task_file1, task_file2):
    ...
    def loss(self, modal_x, modal_y, label, label_x, label_y, Mask):
        ...
        for stage_id in range(self.num_stages):
            ...
            fuse_xy_vis = self.task2.eencoders[stage_id].forward_features(feat_x_vis, feat_y_vis)

            # feature search
            search_feat = self.nddrs[stage_id](fuse_xy_sem, fuse_xy_vis)
            fuse_xy_sem = self.task1.brige[stage_id](fuse_xy_sem)
            fuse_xy_vis = self.task2.brige[stage_id](fuse_xy_vis)

            out_semantic.append(fuse_xy_sem)
            out_visual.append(fuse_xy_vis)

        result.out1 = self.task1.decoder.forward(out_semantic)
        result.out2 = self.task1.decoder.forward(out_visual)

        result.loss1 = self.task1.loss(result.out1, label)
        result.loss2 = self.task2.loss(result.out2, label_x, label_y, Mask)
